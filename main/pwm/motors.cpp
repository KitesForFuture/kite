...
static uint32_t escDegree2pulsewidth(uint32_t degree_of_rotation) {
    ...
    cal_pulsewidth = (ESC_MIN_PULSEWIDTH +
                      (((ESC_MAX_PULSEWIDTH - ESC_MIN_PULSEWIDTH) * (degree_of_rotation)) / (ESC_MAX_DEGREE)));
...
static uint32_t servoDegree2pulsewidth(int32_t degree_of_rotation) {
    ...
    cal_pulsewidth = (SERVO_MIN_PULSEWIDTH +
                      (((SERVO_MAX_PULSEWIDTH - SERVO_MIN_PULSEWIDTH) * (degree_of_rotation + 90)) / (180)));
    return cal_pulsewidth;
}
