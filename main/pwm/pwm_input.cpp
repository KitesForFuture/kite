...
int pwm_input_value[NUM_PWM_INPUT_CHANNELS] = {PWM_INPUT_MIN_DUTY, PWM_INPUT_MIN_DUTY, PWM_INPUT_MIN_DUTY,
                                               PWM_INPUT_MIN_DUTY};
...
void updatePWMInput() {
    ...
    for (int i = 0; i < num_registered_channels; i++) {
        ...
        while (1) {
            ...
            rmt_item32_t *items = (rmt_item32_t *) xRingbufferReceive(pwm_buffer_handles[i],
                                                                      &length/*will be 4 (meaning 4 bytes, i.e. 32 bits)*/,
                                                                      1/*if too high buffer runs full for long pulses?*/);
            if (items) {
