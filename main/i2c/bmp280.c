...
int bmp280_update_if_possible() { 
  if (query_timer_microseconds(last_update) >= UPDATE_INTERVAL_MICROSECONDS) {
    ...
    current_smoothened_temperature = ((float)get_temperature() * SMOOTHING_TEMPERATURE_RECENT_VALUE_WEIGHT) + (current_smoothened_temperature * (1-SMOOTHING_TEMPERATURE_RECENT_VALUE_WEIGHT));
    current_smoothened_pressure = (get_pressure() * SMOOTHING_PRESSURE_RECENT_VALUE_WEIGHT) + (current_smoothened_pressure * (1-SMOOTHING_PRESSURE_RECENT_VALUE_WEIGHT));
